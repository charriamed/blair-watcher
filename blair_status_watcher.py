...
def color_for(status):
    return {
        "operational": 5763719,   # Vert
        "degraded": 16763904,     # Jaune
        "maintenance": 16098851,  # Orange
        "major_outage": 15548997, # Rouge
        "unknown": 9807270,       # Gris
        "not_found": 10197915,    # Bleu
    }.get(status, 9807270)
...
def format_blair_message(current):
    ...
    if not current["found"]:
        return f"❓ **Blair** introuvable dans la liste DOFUS Touch"
    
    emoji = "🟢" if status == "operational" else "🟡" if status == "degraded" else "🔧" if status == "maintenance" else "🔴"
...
def run_once(notify_on_first_run=False):
    ...
    if previous_status is None:
        ...
        if notify_on_first_run:
            send_webhook(format_blair_message(current), color_for(current.get('status', 'unknown')))
        return
